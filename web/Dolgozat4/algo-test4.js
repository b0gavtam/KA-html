...
function deEmojify(emojiString) {
  ...
  const getStringNum = emojiString.split(' ').map(e => Number(smilesMap[e]));
  const getCharFromNum = getStringNum.map(e => e)
  
  
return getCharFromNum.map((e) => {
  if (e === 0 || e >= 0) {
    return e;
  }
  
  return ' '
}).join('').split(' ').map(e => String.fromCharCode(e)).join('') || null
}
console.log('\n3. feladat:')
